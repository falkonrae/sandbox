...
/*   Updated: 2021/01/22 11:16:08 by uteena           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			get_next_line(int fd, char **line)
{
	...
	while ((read_return = read(fd, buffer, BUFFER_SIZE)) >= 0)
	{
		...
		make_draft(&draft_line[fd], &buffer);
		if (ft_strchr(draft_line[fd], '\n'))
			return (make_line(line, &draft_line[fd], &buffer));
		if (read_return == 0)
		{
			free(buffer);
			buffer = NULL;
			return (gnl_end(line, &draft_line[fd]));
		}
	}
	*line = NULL;
	return (-1);
